...
const vehicleList = ref<Vehicle[]>(
  props.vehicles?.length ? [...props.vehicles] : props.useDemoWhenEmpty ? [...demoVehicles] : [],
);
const currentDefaultId = ref<string | null>(
  props.defaultId ||
    vehicleList.value.find((v) => v.isDefault)?.id ||
    vehicleList.value[0]?.id ||
    null,
);
  ...
  <section class="w-full bg-[#EBE8E3]">
    <div class="mx-auto max-w-4xl px-5 py-8">
      ...
      <div class="flex flex-col gap-6">
        <article v-for="v in vehicleList" :key="v.id" class="relative px-6 py-6 overflow-hidden bg-white rounded-[24px] shadow-sm">
          ...
          <div class="flex items-start gap-5 mb-6">
            ...
            <div class="flex flex-col pt-1">
              ...
              <div class="mt-3">
                <span class="inline-block px-4 py-1.5 text-base font-bold tracking-wider text-[#2F2F2F] rounded-xl bg-[#E7E1DA]">
                  {{ v.plate }}
                </span>
              </div>
            </div>
    ...
    <div class="fixed right-6 bottom-6 z-50 flex flex-col gap-3">
      ...
      >
        <span class="material-symbols-outlined text-[24px]" :class="{ 'text-[#4CAF50]': toast.type === 'success', 'text-[#F44336]': toast.type === 'error', 'text-[#2196F3]': toast.type === 'info',}">
          {{ toast.type === 'success' ? 'check_circle' : toast.type === 'error' ? 'error' : 'info' }}
        </span>
        <span class="text-base font-medium text-[#2F2F2F]">{{ toast.message }}</span>
      </div>
    </div>
    <div v-if="deleteModal.show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-md" @click.self="cancelDelete">
    ...
    <div v-if="editModal.show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-md" @click.self="cancelEdit">
      <div class="bg-white rounded-[24px] p-8 max-w-md w-full mx-4 shadow-2xl">
        ...
        <div class="flex gap-3">
          ...
          <button
            class="flex-1 px-6 py-3 text-base font-bold text-white transition-colors rounded-full bg-[#6B6B5C] hover:bg-[#5a5a4d]"
            @click="saveEdit"
          >
            儲存
          </button>
