...
const shopHeaderInfo = computed(() => {
  ...
  const items = [
    ...
    {
      label: '目前狀態',
      value: getStatusLabel(props.member.status),
      isTag: true,
      tagClass: getStatusClass(props.member.status),
    },
  ];
...
<template>
  ...
  >
    ...
    <!-- Modal 主體 -->
    <article
      class="relative w-full max-w-2xl bg-white rounded-lg shadow-xl border border-gray-200 flex flex-col max-h-[90vh] overflow-hidden transition-all"
    >
      <!-- 標題區 -->
      ...
      <section class="p-6 overflow-y-auto custom-scrollbar flex-1">
        ...
        <div
          v-if="type !== 'carOwner'"
          class="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-100 mb-4"
        >
          <div v-for="item in shopHeaderInfo" :key="item.label">
            <span class="text-xs text-gray-500 block uppercase tracking-wide">{{
              item.label
            }}</span>
            <span
              v-if="item.isTag"
              class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs mt-1"
              :class="item.tagClass"
              >{{ item.value }}</span
            >
      ...
      <!-- 底部按鈕區 -->
      <footer
        class="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row-reverse border-t border-gray-200 gap-2"
      >
        <button
          ...
          type="button"
          @click="btn.onClick"
          class="w-full sm:w-auto px-4 py-2 rounded-md border shadow-sm text-sm font-medium transition-colors"
          :class="btn.class"
        >
          {{ btn.label }}
        </button>
