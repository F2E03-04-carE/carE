...
interface DisplayField {
  label: string;
  value: string | number | MemberStatus | undefined;
  isStatus?: boolean;
  colSpan?: boolean;
}
...
<template>
  <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        >
          ...
          <span
            ...
            :class="getStatusClass(field.value as MemberStatus)"
          >
            {{ getStatusLabel(field.value as MemberStatus) }}
          </span>
    ...
    <div v-if="member.vehicles && member.vehicles.length" class="space-y-2">
      <!-- 迴圈渲染每一輛登記的車輛 -->
      <div
        v-for="car in member.vehicles"
        :key="car.plate"
        class="flex items-center justify-between p-3 border border-gray-200 rounded-md bg-white shadow-sm hover:shadow-md transition-shadow"
      >
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500">
              {{ car.year }}年 •
              <span class="bg-gray-100 px-1 rounded text-gray-600">{{ car.plate }}</span>
            </p>
          </div>
        </div>
      </div>
    </div>
    <p v-else class="text-sm text-gray-500 italic p-2 text-center bg-gray-50 rounded">
      尚無車輛資料
    </p>

    <h4
      class="font-medium text-gray-800 mt-5 mb-2 border-l-4 border-[#6b6b5a] pl-2 flex items-center"
    >
    ...
    <ul v-if="member.bookings && member.bookings.length" class="space-y-2">
      ...
      <li
        v-for="booking in member.bookings"
        :key="booking.id"
        class="flex items-center text-sm text-gray-600 bg-white p-2 rounded border border-gray-100"
      >
