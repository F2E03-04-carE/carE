...
const emit = defineEmits<{
  ...
  (event: 'save', status: OrderStatus): void;
}>();
      ...
      <div class="space-y-6">
        ...
        <div class="rounded-xl bg-white p-5">
          ...
          <div v-if="order?.note" class="mt-4">
            <p class="text-sm text-[#8a8a7d] mb-1 font-medium">客戶備註</p>
            <div
              class="w-full rounded-xl bg-[#f5f4f0] p-3 text-sm text-[#4a4a43] whitespace-pre-wrap"
            >
              {{ order.note }}
            </div>
          </div>
