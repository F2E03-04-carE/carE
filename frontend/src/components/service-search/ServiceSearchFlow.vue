...
<template>
  <div class="min-h-screen bg-[#FAF8F5] px-6 py-10 text-[#2B2B2B]">
    <div class="mx-auto mb-6 w-full max-w-6xl">
      <button type="button" class="flex items-center gap-2 rounded-xl border border-[#E6E6DF] bg-white px-6 py-3 text-sm font-semibold text-[#6B6B5C] transition hover:bg-[#F5F4EF]" @click="$emit('back')">
    ...
    <div class="mx-auto w-full max-w-6xl rounded-[28px] border border-[#E6E6DF] bg-white shadow-[0_2px_10px_rgba(0,0,0,0.06)]">
      <div class="p-10">
        ...
        <div class="mt-16">
          <div v-if="step === 1">
            ...
            <div class="mt-10 grid grid-cols-2 gap-6">
              <button v-for="item in serviceOptions" :key="item.key" type="button" class="flex items-center gap-5 rounded-2xl border px-7 py-6 text-left transition" :class="serviceCardClass(item.key)" @click="selectedService = item.key">
                <div class="grid h-14 w-14 place-items-center rounded-2xl transition" :class=" selectedService === item.key ? 'bg-white/20 text-white' : 'bg-[#F5F4EF] text-[#6B6B5C]'">
                  <!-- <i :class="item.fa" class="text-xl" aria-hidden="true"></i> -->
                  <span class="material-symbols-outlined">{{ item.icon }}</span>
                </div>
                <div class="min-w-0">
                  <div class="text-lg font-semibold transition" :class="selectedService === item.key ? 'text-white' : 'text-[#2B2B2B]'">
                    {{ item.title }}
                  </div>
                </div>
              </button>
            ...
            <div class="mt-10 rounded-2xl border border-[#E6E6DF] bg-white px-10 py-6">
              <div class="grid grid-cols-[44px_1fr] gap-6 py-7">
                ...
                <div>
                  ...
                  <input v-model.trim="contactName" class="mt-3 h-11 w-full rounded-xl bg-[#F5F4EF] px-4 text-sm outline-none placeholder:text-[#B5B5AD]" placeholder="請輸入姓名"/>
                </div>
              </div>
              ...
              <div class="grid grid-cols-[44px_1fr] gap-6 py-7">
                ...
                <div>
                  ...
                  <input v-model.trim="contactPhone" inputmode="tel" class="mt-3 h-11 w-full rounded-xl bg-[#F5F4EF] px-4 text-sm outline-none placeholder:text-[#B5B5AD]" placeholder="請輸入電話"/>
                </div>
              </div>
              <div class="h-px bg-[#E6E6DF]" />
              <div class="grid grid-cols-[44px_1fr] gap-6 py-7">
                ...
                <div>
                  ...
                  <input v-model.trim="contactEmail" inputmode="email" class="mt-3 h-11 w-full rounded-xl bg-[#F5F4EF] px-4 text-sm outline-none placeholder:text-[#B5B5AD]" placeholder="請輸入電子郵件"/>
                </div>
              </div>
              <div class="h-px bg-[#E6E6DF]" />
              <div class="grid grid-cols-[44px_1fr] gap-6 py-7">
                ...
                <div>
                  ...
                  <textarea v-model.trim="contactNote" rows="3" class="mt-3 w-full resize-none rounded-xl bg-[#F5F4EF] px-4 py-3 text-sm outline-none placeholder:text-[#B5B5AD]" placeholder="如有特殊需求請在此填寫"/>
                </div>
              </div>
            </div>
          </div>
              ...
              <div v-for="(item, index) in summaryItems" :key="item.key">
                <div class="grid grid-cols-[44px_1fr] gap-6 py-8">
                  ...
                  <div>
                    ...
                    <div v-if="item.details" class="mt-2" :class="item.detailsClass || 'text-sm text-[#7A7A7A]'">
                      {{ item.details }}
                    </div>
                  </div>
                </div>
        ...
        <div class="mt-14 flex items-center justify-between">
          <button type="button" class="rounded-xl border px-10 py-3 text-sm font-semibold transition" :class="prevBtnClass" :disabled="step === 1" @click="prevStep">
            上一步
          </button>
          <button v-if="step < 4" type="button" class="rounded-xl px-10 py-3 text-sm font-semibold text-white transition" :class="nextBtnClass" :disabled="!canGoNext" @click="nextStep">
            下一步
          </button>
          <button v-else type="button" class="rounded-xl bg-[#6B6B5C] px-10 py-3 text-sm font-semibold text-white shadow-[0_3px_0_rgba(0,0,0,0.18)] hover:opacity-95 active:opacity-90" :disabled="!canFinish" @click="finish">
            完成預約
          </button>
        </div>
      </div>
