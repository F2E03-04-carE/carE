...
<template>
  <div class="relative w-full">
    ...
    <div v-if="step > 1" class="absolute left-0 top-[18px] h-px bg-[#6B6B5C] transition-all duration-300" :style="{ width: progressWidth }"/>
    <div class="grid grid-cols-4 items-start gap-0">
      <button
        ...
        @click="$emit('go', idx + 1)"
      >
        <span class="grid h-9 w-9 place-items-center rounded-full text-sm font-semibold transition z-10" :class="circleClass(idx + 1)">
