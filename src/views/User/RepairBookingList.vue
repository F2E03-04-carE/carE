...
<template>
	<div class="min-h-screen bg-[#F4F1EC] text-[#2F2E2A]">
		<header class="bg-[#F4F1EC] border-b border-[#E2DED6]">
			<div class="container mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-5">
				<div>
					...
					<p class="mt-1 text-sm text-[#6B6B5C]">查看與管理你的汽車維修預約</p>
				</div>
				<div class="flex items-center gap-3">
					<button type="button" class="relative inline-flex items-center justify-center gap-2 rounded-xl bg-[#D9D6CF] px-4 py-2 text-sm text-[#2F2E2A] transition-colors duration-200 hover:bg-[#CFCBC3]">
					...
					<button type="button" class="flex items-center justify-center gap-2 rounded-lg bg-[#D9D6CF] px-3 py-2 text-sm text-[#2F2E2A] transition-colors duration-200 hover:bg-[#CFCBC3]">
		...
		<main class="container mx-auto max-w-6xl px-4 py-8">
			<div class="mb-7">
				<div class="flex flex-wrap gap-2">
					<button
						...
						:class="[
							`inline-flex items-center justify-center rounded-xl border border-[#E2DED6] px-4 py-2 text-sm transition-colors duration-200`,
			...
			<div v-if="FilteredAppointments.length > 0" class="grid grid-cols-1 gap-6">
				<div
					...
					:key="Appointment.id"
					class="rounded-2xl border border-[#E2DED6] bg-[#F7F5F0] p-6 transition-colors duration-200 hover:bg-[#F7F5F0]"
				>
					<div class="flex flex-col gap-5">
						<div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
							<div class="min-w-0">
								<div class="mb-2 flex items-center gap-2">
									<h3 class="truncate font-semibold text-[#2F2E2A]">{{ Appointment.carModel }}</h3>
									<span class="text-sm text-[#6B6B5C]">・{{ Appointment.licensePlate }}</span>
								</div>
								<div class="mb-1 flex items-center gap-2 text-sm text-[#6B6B5C]">
						...
						<div class="grid grid-cols-1 gap-3 border-y border-[#E2DED6] py-4 sm:grid-cols-2">
							<div class="flex items-center gap-2 text-sm">
								<font-awesome-icon :icon="['fas', 'calendar-days']" class="h-4 w-4 text-[#6B6B5C]" />
								<div>
									<p class="text-xs text-[#6B6B5C]">預約日期</p>
									<p class="text-[#2F2E2A]">{{ Appointment.appointmentDate }}</p>
								</div>
							</div>
							<div class="flex items-center gap-2 text-sm">
								<font-awesome-icon :icon="['fas', 'clock']" class="h-4 w-4 text-[#6B6B5C]" />
								<div>
									<p class="text-xs text-[#6B6B5C]">預約時間</p>
									<p class="text-[#2F2E2A]">{{ Appointment.appointmentTime }}</p>
								</div>
							</div>
							<div class="flex items-center gap-2 text-sm">
								<font-awesome-icon :icon="['fas', 'location-dot']" class="h-4 w-4 text-[#6B6B5C]" />
								<div>
									<p class="text-xs text-[#6B6B5C]">門市地點</p>
									<p class="line-clamp-1 text-[#2F2E2A]">{{ Appointment.location }}</p>
								</div>
							</div>
							<div class="flex items-center gap-2 text-sm">
								<font-awesome-icon :icon="['fas', 'car']" class="h-4 w-4 text-[#6B6B5C]" />
								<div>
									<p class="text-xs text-[#6B6B5C]">預估時間</p>
			...
			<div v-else class="rounded-2xl border border-[#E2DED6] bg-[#F7F5F0] p-12 text-center">
				...
				<h2 class="mb-2 text-lg font-semibold text-[#2F2E2A]">沒有符合條件的預約</h2>
				<p class="text-[#6B6B5C]">請切換篩選條件,或稍後再試。</p>
			</div>
		</main>
		<Teleport to="body">
			<div v-if="CancelDialogOpen" class="fixed inset-0 z-50">
				<div class="absolute inset-0 bg-black/50" @click="CloseCancelDialog"></div>
				<div class="absolute inset-0 flex items-center justify-center p-4">
					<div class="w-full max-w-md rounded-2xl border border-[#E2DED6] bg-[#F7F5F0] p-6 shadow-lg">
						<h3 class="text-lg font-semibold text-[#2F2E2A]">取消預約</h3>
						<p class="mt-2 text-sm text-[#6B6B5C]">
							確定要取消這筆預約嗎?取消後可能需要重新預約時間。
						</p>
						<div v-if="SelectedAppointment" class="mt-4 rounded-xl bg-[#EFECE6] p-4 text-sm">
							<p class="font-medium text-[#2F2E2A]">{{ SelectedAppointment.carModel }}</p>
							<p class="mt-1 text-[#6B6B5C]">
						...
						<div class="mt-6 flex gap-3">
							<button
								...
								class="flex-1 rounded-lg border border-[#E2DED6] bg-[#D9D6CF] px-4 py-2.5 text-sm text-[#2F2E2A] transition-colors duration-200 hover:bg-[#CFCBC3]"
							>
								返回
							</button>
							<button
								...
								class="flex-1 rounded-lg bg-[#B85C50] px-4 py-2.5 text-sm text-white transition-colors duration-200 hover:bg-[#A04B40]"
							>
								確認取消
							</button>
						</div>
				...
				<div class="absolute inset-0 flex items-center justify-center p-4">
					<div class="w-full max-w-2xl rounded-2xl border border-[#E2DED6] bg-[#F7F5F0] p-6 shadow-lg">
						<div class="flex items-center justify-between gap-4">
							<h3 class="text-lg font-semibold text-[#2F2E2A]">預約詳情</h3>
							<span
								class="rounded-full border px-3 py-1.5 text-sm"
						...
						<div class="mt-4 space-y-6">
							<div class="rounded-lg bg-[#F4F1EC]/50 p-4">
								<h4 class="mb-3 flex items-center gap-2 font-medium text-[#2F2E2A]">
									<font-awesome-icon :icon="['fas', 'car']" class="h-4 w-4 text-[#6B6B5C]" />
									車輛資訊
								</h4>
								<div class="grid grid-cols-2 gap-3 text-sm">
									<div>
										<p class="mb-1 text-xs text-[#6B6B5C]">車型</p>
										<p class="text-[#2F2E2A]">{{ SelectedAppointment.carModel }}</p>
									</div>
									<div>
										<p class="mb-1 text-xs text-[#6B6B5C]">車牌號碼</p>
							...
							<div class="rounded-lg bg-[#F4F1EC]/50 p-4">
								...
								<div class="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
									...
									<div class="flex items-center gap-2 sm:col-span-2">
										<font-awesome-icon :icon="['fas', 'wrench']" class="h-4 w-4 text-[#6B6B5C]" />
										<div>
											<p class="text-xs text-[#6B6B5C]">項目</p>
							...
							<div class="rounded-lg bg-[#F4F1EC]/50 p-4">
								<h4 class="mb-3 flex items-center gap-2 font-medium text-[#2F2E2A]">
									<font-awesome-icon :icon="['fas', 'user']" class="h-4 w-4 text-[#6B6B5C]" />
									客戶資訊
								</h4>
								<div class="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
									<div class="flex items-center gap-2">
										<font-awesome-icon :icon="['fas', 'user']" class="h-4 w-4 text-[#6B6B5C]" />
										<div>
											<p class="text-xs text-[#6B6B5C]">姓名</p>
											<p class="text-[#2F2E2A]">{{ SelectedAppointment.customerName }}</p>
										</div>
									</div>
									<div class="flex items-center gap-2">
										<font-awesome-icon :icon="['fas', 'phone']" class="h-4 w-4 text-[#6B6B5C]" />
										<div>
											<p class="text-xs text-[#6B6B5C]">電話</p>
							...
							<div class="rounded-lg bg-[#F4F1EC]/50 p-4">
								<h4 class="mb-3 flex items-center gap-2 font-medium text-[#2F2E2A]">
									<font-awesome-icon :icon="['fas', 'screwdriver-wrench']" class="h-4 w-4 text-[#6B6B5C]" />
									服務項目
								</h4>
								<p class="text-sm text-[#2F2E2A]">{{ SelectedAppointment.serviceType }}</p>
								<div class="mt-3 border-t border-[#E2DED6] pt-3 text-sm">
									<div class="flex items-center gap-2">
										<font-awesome-icon :icon="['fas', 'dollar-sign']" class="h-4 w-4 text-[#6B6B5C]" />
										<span class="text-xs text-[#6B6B5C]">預估費用</span>
										<span class="ml-auto text-[#2F2E2A]">NT$ {{ SelectedAppointment.estimatedCost }}</span>
									</div>
								</div>
								<div v-if="SelectedAppointment.additionalServices.length" class="mt-3 flex flex-wrap gap-2">
									<span
										...
										:key="Index"
										class="rounded-full bg-[#D9D6CF] px-3 py-1 text-xs text-[#2F2E2A]"
									>
										{{ Service }}
									</span>
								</div>
							...
							<div class="rounded-lg bg-[#F4F1EC]/50 p-4">
								...
								<div class="space-y-3 text-sm">
									<div>
										<p class="mb-1 text-xs text-[#6B6B5C]">門市名稱</p>
										<p class="text-[#2F2E2A]">{{ SelectedAppointment.location }}</p>
									</div>
								</div>
							</div>
							<div v-if="SelectedAppointment.notes" class="rounded-lg bg-[#F4F1EC]/50 p-4">
								<h4 class="mb-3 flex items-center gap-2 font-medium text-[#2F2E2A]">
									<font-awesome-icon :icon="['fas', 'file-lines']" class="h-4 w-4 text-[#6B6B5C]" />
									備註
								</h4>
						...
						<div class="mt-6 flex justify-end">
							<button
								type="button"
								@click="CloseDetail"
								class="rounded-lg border border-[#E2DED6] bg-[#D9D6CF] px-5 py-2.5 text-sm text-[#2F2E2A] transition-colors duration-200 hover:bg-[#CFCBC3]"
							>
								關閉
							</button>
						</div>
