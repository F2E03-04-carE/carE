      ...
      <form @submit.prevent="handleLoginSubmit" class="flex flex-col gap-4">
        <div class="flex flex-col gap-1">
          ...
          <input
            ...
            type="text"
            placeholder="請輸入您的帳號"
            :class="['w-full px-4 py-2 text-[16px] rounded-lg border outline-none transition-all', formErrors.account ? 'border-red-500 bg-red-50' : 'border-gray-300 focus:border-[#6b6b5a]']"
          />
          <span v-if="formErrors.account" class="text-[14px] text-red-500">{{ formErrors.account }}</span>
        </div>
        <div class="flex flex-col gap-1">
          ...
          <input
            ...
            type="password"
            placeholder="請輸入您的密碼"
            :class="['w-full px-4 py-2 text-[16px] rounded-lg border outline-none transition-all', formErrors.password ? 'border-red-500 bg-red-50' : 'border-gray-300 focus:border-[#6b6b5a]']"
          />
          <span v-if="formErrors.password" class="text-[14px] text-red-500">{{ formErrors.password }}</span>
        </div>
        <div class="flex justify-between items-center mt-1">
          <label class="flex items-center gap-2 cursor-pointer select-none">
            <input v-model="loginForm.rememberMe" type="checkbox" class="w-4 h-4 text-[#6b6b5a] rounded border-gray-300 focus:ring-[#6b6b5a]" />
            <span class="text-[14px] text-gray-600">記住帳號密碼</span>
          </label>
          <a href="#" class="text-[14px] text-[#6b6b5a] hover:underline hover:text-[#4a4a43]">忘記密碼？</a>
        </div>
        <button type="submit" class="mt-2 w-full py-2.5 text-[16px] font-bold text-white bg-[#6b6b5a] rounded-lg hover:bg-[#5a5a4a] transition-colors shadow-md cursor-pointer">登入</button>
      </form>
